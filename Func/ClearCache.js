        ...
        if (!Args.New || utils.getType(Args.New) !== "Array") { 
            ...
            for (let i = 0; i < Args.New.length; i++) {
                if (Object.indexOf(Args.New[i]) === -1) {
                    log.Normal('file not found ' + Args.New[i] + 'in This format list');
                    return;
                }
        ...
        switch (process.platform) {
            case 'linux': {
                for (let i = 0; i < New1.length; i++) {
                    log.Normal(' Clear  File ' + New1[i]);
                    var STR = String(`find ./modules -type f -iname \'*.${New1[i]}\' -exec rm {} \\;`);
                    execSync(STR);
                }
                log.Normal(' Clear ' + New1.length + '  File !');
                callback(null, ' Clear ' + New1.length + '  File !');
            }
            break;
            case "win32": {
                var cmd = "del /q /s /f /a ";
